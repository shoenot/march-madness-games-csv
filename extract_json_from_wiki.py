...
def extract_json_from_wiki(inpath, outpath):
    source = open(inpath).read()
    bracket = extract_bracket(source, extract_year(inpath.name))
    json.dump(bracket, open(outpath, "w"))
